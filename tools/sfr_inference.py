...
def parse_args():
    ...
    parser.add_argument(
        '--dataset', default='gid', choices=['gid', 'fbps'],
        help='dataset preset')
    parser.add_argument(
        '--src', default='data/GID/Image_test',
        help='folder containing ultra-wide area images')
    parser.add_argument(
        '--dst', default='work_dirs/sfr_predictions',
        help='folder to save prediction masks')
    parser.add_argument(
        '--config', default='configs/gid/sfrnet_swinl_320k_gid.py',
...
def build_scale_frustum(img,
                        ...
                        use_global=True,
                        load_type='random'):
    """Build Scale-Frustum Representations for one projection reference point."""
...
def sfr_infer_single(img,
    ...
    if load_type in ['random', 'local']:
        ...
        pred = cv2.resize(
            pred_mask, dsize=img.shape[:2], interpolation=cv2.INTER_NEAREST)
...
def main():
    args = parse_args()
    distances = [1, 3, 14]
    num_classes = 6 if args.dataset == 'gid' else 25
    sfr_infer(
        args.src, args.dst, args.config, args.ckpt, distances, args.stride,
        num_classes, args.load_type, args.reverse)
